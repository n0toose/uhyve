...
/// - The return value is only valid, as long as the guest is halted.
/// - This fn must not be called multiple times on the same data, to avoid creating mutable aliasing.
pub unsafe fn address_to_hypercall_v2<'a>(
	mem: &'a MmapMemory,
	addr: u16,
	data: GuestPhysAddr,
) -> Option<v2::Hypercall<'a>> {
		...
		if let Ok(host_address) = mem.host_address(sysread.buf) {
			match fdata {
				FdData::Raw(rfd) => {
					...
					if bytes_read <= 0 {
						sysread.ret = -translate_last_errno().unwrap_or(1) as i64;
					}
					bytes_read as i64
				}
			...
			sysread.ret = -EFAULT as i64;
			0
		}
	} else {
		sysread.ret = -EBADF as i64;
		0
	};
}
...
pub fn write_v1(
	...
	// Error number is silently ignored for v1, len used to be overwritten.
	if tmp.ret > 0 {
		syswrite.len = tmp
			.ret
			.try_into()
			.unwrap_or_else(|ret| panic!("Unable to fit return value {} in write_v1.", ret));
	}
	res
}
